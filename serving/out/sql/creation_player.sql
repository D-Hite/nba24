CREATE OR REPLACE TABLE processed.PLAYER_10_AVG_TABLE AS
	...
	,AVG(COALESCE(OPP_OREB_PCT,0)) OVER (
        ...
        ORDER BY GAME_DATE
        ROWS BETWEEN 10 PRECEDING AND 1 PRECEDING
    ) AS rolling_10_avg_OPP_OREB_PCT
	,AVG(COALESCE(USG_PCT,0)) OVER (
        PARTITION BY PLAYER_NAME, SEASON_ID 
        ORDER BY GAME_DATE
        ROWS BETWEEN 10 PRECEDING AND 1 PRECEDING
    ) AS rolling_10_avg_USG_PCT,COUNT(*) OVER (
        PARTITION BY PLAYER_NAME, SEASON_ID
        ORDER BY GAME_DATE
	...
	,rolling_10_avg_USG_PCT as USG_PCT
    ,GAME_COUNT
    FROM INTERMEDIATE_DATA
    --WHERE SEASON_ID::varchar not like '4%'
    ORDER BY SEASON_ID, TEAM_ABBREVIATION, GAME_DATE

CREATE OR REPLACE TABLE processed.PLAYER_10_PER_MIN_AVG_DATA AS
    with INTERMEDIATE_DATA as (
        SELECT
        TEAM_ABBREVIATION
        ,PLAYER_NAME
